...
const rows: RecordRow[] = [
  {id:'alpha',name:'Primary certificate paths',revision:3,content:'certificate paths: alpha\nstate: active',updatedAt:new Date(0).toISOString()},
  {id:'beta',name:'Secondary certificate paths',revision:5,content:'certificate paths: beta\nstate: review',updatedAt:new Date(1000).toISOString()},
];

export function createApp(){
  const app=express();
  app.use(express.json({limit:'1mb'}));
  app.get('/api/bootstrap',(_req,res)=>res.json({family:"cert-chain",count:rows.length}));
  app.get('/api/chains',(_req,res)=>res.json(rows.map(({content,...row})=>row)));
  app.get('/api/chains/:id',(req,res)=>{const row=rows.find(value=>value.id===req.params.id);if(!row)return res.status(404).json({error:'not_found'});res.set('ETag',String(row.revision)).json(row)});
